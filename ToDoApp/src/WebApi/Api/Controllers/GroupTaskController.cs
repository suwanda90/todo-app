...
{
    public class GroupTaskController : BaseApiController
    {
        private readonly IGroupTaskService _service;
        private readonly IAppLogger<GroupTask> _loger;

        public GroupTaskController(IGroupTaskService service, IAppLogger<GroupTask> loger)
        {
            _service = service;
            _loger = loger;
        }
        ...
        [HttpPut]
        public async Task<IActionResult> PutAsync([FromBody] GroupTask data)
        {
            if (data == null)
            {
                _loger.LogError("Bad request parameter");
        ...
        public async Task<IActionResult> IsExistValueWithKeyAsync([FromBody] ExistWithKeyModel model)
        {
            ...
            else if (typeof(GroupTask).HasProperty(model.KeyName.ToLower()) && typeof(GroupTask).HasProperty(model.FieldName.ToLower()))
            {
